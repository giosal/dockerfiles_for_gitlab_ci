...
if [[ $PHP_VERSION == "8.5" ]]; then
  ...
  docker-php-ext-install -j "$(nproc)" exif pcntl bcmath bz2 calendar intl mysqli opcache pdo_pgsql pgsql pdo_sqlite soap xsl gmp && \
...
if [[ $PHP_VERSION == "8.5" || $PHP_VERSION == "7.4" ]]; then
